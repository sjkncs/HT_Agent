...
public class LLMServiceImpl implements LLMService {
    ...
    static {
        // ── Food Safety: 来自 关键词.xlsx Sheet1 的 113 条规则 ──
        FOOD_SAFETY_PATTERNS = compilePatterns(
                // 外源性异物 (regex from 关键词.xlsx 匹配1-5)
                "(异物|头发|毛发|塑料|金属|玻璃|虫|苍蝇|蟑螂|纸片|线头|棉絮|小虫|飞虫)",
                "(喝出|吃出|吸出).{0,4}(虫|发|毛|塑料|金属|玻璃|纸|线|棉)",
                "(不明物|黑色|黑的|颗粒物|沉淀物|不明物体)",
                "(封口标签|封口贴|标签|贴纸).{0,5}(掉|落|进|在)",
                // 内源性异物
                "(果核|籽|茶渣|果皮|果肉|柠檬皮|柠檬籽|芒果核|芒果皮)",
                "(纤维|果肉.{0,2}块|水果纤维|茶叶.{0,2}渣)",
                // 身体不适 (from 关键词.xlsx 过敏/呕吐/腹泻/其它不适)
                "(拉肚子|腹泻|呕吐|过敏|恶心|头晕|发烧|不舒服|肚子疼|肚子痛)",
                "(上吐下泻|拉肚|吃完不舒服|喝了以后|喝完.*不舒服|食物中毒)",
                "(皮疹|红疹|瘙痒|胃痛|胃疼|肠胃|食物中毒)",
                // 变质/异味 (from 关键词.xlsx 原料变质/产品有效期)
                "(变质|发霉|过期|馊|酸了|酸酸的|异味|怪味|味道不对|一股味道)",
                "(有股|变酸|变味|发酸|发臭|腐烂|腐坏|地沟油|品控|品质问题)",
                "(保质期|临期|有效期|生产日期|过了.*期)",
                // 投诉/维权
                "(退款|赔偿|补偿|投诉|曝光|差评|给我一个说法|举报|工商|消协|12315)",
                // 包装/卫生
                "(包装破|漏杯|撒了|封口不严|食安|食品安全|卫生问题|不干净|脏)",
                "(杯盖.{0,3}(裂|破|坏|掉)|吸管.{0,3}(脏|破|坏))",
                // 英文
    ...
    private String detectFoodSafetySubScenario(String msg) {
        ...
        if (matchAny(msg, compilePatterns(
                "(拉肚子|腹泻|呕吐|过敏|恶心|头晕|发烧|肚子疼|肚子痛|上吐下泻|身体不适|不舒服|食物中毒|皮疹|胃)"))) {
            return "body_discomfort";
        }
        // 变质/过期 → critical
        if (matchAny(msg, compilePatterns(
                "(变质|发霉|过期|馊|变酸|发臭|腐|保质期|临期|有效期)"))) {
            return "spoilage";
        }
        // 外源性异物 → high
        if (matchAny(msg, compilePatterns(
                "(异物|头发|毛发|塑料|金属|玻璃|虫|苍蝇|蟑螂|纸片|线头|棉絮|不明物|黑的|封口标签|封口贴)"))) {
            return "foreign_object_external";
        }
        // 内源性异物 → medium
        if (matchAny(msg, compilePatterns(
                "(果核|籽|茶渣|果皮|果肉|柠檬皮|柠檬籽|芒果核|芒果皮|纤维)"))) {
            return "foreign_object_internal";
        }
        // 异味/口感异常 → medium
        if (matchAny(msg, compilePatterns(
                "(异味|怪味|味道不对|一股味道|有股|酸酸的|酸了|味)"))) {
            return "taste_issue";
        }
